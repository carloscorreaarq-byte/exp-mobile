...
// Gerado uma vez com: npx web-push generate-vapid-keys
// Coloque a PUBLIC key aqui após gerar
...
function roleLabel(role) {
  return { socio: 'Sócio', socio_admin: 'Sócio Admin', coordenador: 'Coordenador', colaborador: 'Colaborador' }[role] ?? role;
}
...
async function _loadChatBadge() {
  ...
  try {
    ...
    const since = rd?.last_read_at ?? new Date(0).toISOString();
  ...
  try {
    ...
    if (u) {
      ...
      await sb.from('push_subscriptions').upsert(
        { usuario_id: u.id, endpoint: sub.endpoint, subscription: JSON.stringify(sub) },
